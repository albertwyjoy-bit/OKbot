...
OKbot 支持强大的**定时任务功能**，用**自然语言**描述即可创建周期性任务，由 AI 自动执行或定时发送提醒。

**功能亮点**：
- 📝 **自然语言创建**：直接描述时间和任务，AI 自动处理
- 🤖 **双模式支持**：智能任务执行（Agent 处理）或简单定时提醒
- 📁 **文件自动生成**：任务生成的文件自动上传到飞书
- ⏱️ **Cron 支持**：高级用户可使用 Cron 表达式精确控制
...
- ✅ **全局安装 Chrome DevTools MCP**（用于浏览器控制，无需重复下载）
...
#### 2.1 Kimi Code Plan（**强烈推荐**）

OKbot 基于 Kimi Code CLI，**强烈推荐使用 Kimi Code Plan**，因为内置工具 `SearchWeb` 和 `FetchURL` 依赖 Kimi Code 平台的搜索和抓取服务。
...
根据 [Kimi CLI 官方文档](https://moonshotai.github.io/kimi-cli/zh/customization/agents.html#searchweb)：
...
用于语音消息识别（ASR）和 Midscene 的图像理解能力，也可配置为对话模型。
...
如果希望在对话中使用 GLM 模型而非 Kimi 模型，可以在 `~/.kimi/config.toml` 中添加 GLM 配置：
...
#### 4.1 安装 Chrome DevTools MCP（推荐）

如需控制 Chrome 浏览器，需要**全局安装** chrome-devtools-mcp：

```bash
# 全局安装 Chrome DevTools MCP
npm install -g chrome-devtools-mcp
# 或
pnpm install -g chrome-devtools-mcp
```

> **注意**：全局安装后，启动 OKbot 时无需重复下载，MCP 配置中使用 `"command": "chrome-devtools-mcp"` 即可。

#### 4.2 安装 Midscene Android MCP（可选）
...
创建 `~/.kimi/mcp.json` 配置文件（chrome-devtools-mcp 已在步骤 4.1 全局安装）：
